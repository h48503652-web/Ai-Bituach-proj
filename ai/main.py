...
# # ==========================================
# # 📚 שלב 2: סכמת הנתונים Enterprise של ג'מיני (Confidence + Evidence)
# # ==========================================
...
#     # --- הוספנו כאן את בדיקת אימות הנכס ---
#     is_expected_room: bool = Field(description="CRITICAL: True if the image matches the expected room type requested. False if it is a completely different room.")
#     room_mismatch_reason: Optional[str] = Field(description="If is_expected_room is False, explain what you see instead (e.g., 'Expected Living Room, but saw a Bathroom').")
...
#     invalid_property_reason: Optional[str] = Field(description="If is_valid_property is False, explain what was seen instead (e.g., 'Just a picture of people', 'A dog', etc.).")
...
#     # פרמטרים לבדיקת סיכונים - משודרגים ל-VisionFinding
...
#     # הערכות מספריות - עכשיו מקבלות התייחסות לביטחון המודל
...
# # ==========================================
# # 👁️ שלב 3: ה-Agent של ג'מיני (Vision by Gemini 3.1 Pro)
# # ==========================================
...
#     def build_prompt(self, expected_room: str = "property") -> str:
#         return f"""
#         You are a certified, meticulous insurance field inspector. 
        
#         === ROOM MATCH VALIDATION ===
#         The user was asked to upload a picture of a: **{expected_room}**.
#         CRITICAL: Verify if the image actually shows a {expected_room}. 
#         If they uploaded a kitchen but were asked for a bedroom (or vice versa), set 'is_expected_room' to false and explain why in 'room_mismatch_reason'.

#         CRITICAL FIRST STEP: Validate the media. Does it actually show a property (house, apartment, building exterior/interior)? If it shows completely unrelated content (like random people, faces, animals, screenshots, memes), you MUST set 'is_valid_property' to false and explain why in 'invalid_property_reason'.
...
#         - Provide explicit 'evidence' for EVERY decision. For example, instead of 'Yes', write 'Observed 3 office desks and a waiting area sign'.
#         - Perform a MULTI-PASS analysis: 
#           Pass 1: Identify spaces. 
#           Pass 2: Scan for specific hazards (mold, commercial gear, split units).
#           Pass 3: Self-verify conclusions against evidence.
        
#         You are reviewing visual media. Ideally, look for: Building entrance, Exterior, Front door, Kitchen, Living room, Bedrooms, Garden, Balcony, Pergola, Storage.
...
#     def validate_input_quality(self, files) -> bool:
#         if not files or len(files) < 3: 
#             return False
#         return True

#     # שימי לב - הוספנו כאן את ה-expected_room
#     def process_and_analyze(self, upload_mode, uploaded_files, uploaded_video, expected_room: str = "property"):
#         # שולחים לפרומפט את החדר המבוקש
#         request_parts = [types.Part.from_text(text=self.build_prompt(expected_room))]
#         if upload_mode == "תמונות מרובות" and not self.validate_input_quality(uploaded_files):
#             raise Exception("Input Quality Error: נדרשות לפחות 3 תמונות ברורות לניתוח מהימן.")

#         # שולחים לפרומפט את החדר המבוקש
#         request_parts = [types.Part.from_text(text=self.build_prompt(expected_room))]
#         gemini_uploaded_file = None
        
#         if upload_mode == "סרטון וידאו":
#             with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as tmp_file:
#                 tmp_file.write(uploaded_video.read())
#                 tmp_video_path = tmp_file.name
            
#             gemini_uploaded_file = self.client.files.upload(file=tmp_video_path)
#             while gemini_uploaded_file.state.name == "PROCESSING":
#                 time.sleep(2)
#                 gemini_uploaded_file = self.client.files.get(name=gemini_uploaded_file.name)
                
#             if gemini_uploaded_file.state.name == "FAILED":
#                 raise Exception("ה-AI לא הצליח לעבד את הסרטון (וידאו פגום או לא נתמך).")
                
#             clean_file_part = types.Part(
#                 file_data=types.FileData(file_uri=gemini_uploaded_file.uri, mime_type=gemini_uploaded_file.mime_type)
#             )
#             request_parts.append(clean_file_part)
#             os.remove(tmp_video_path)
#         else:
#             for file in uploaded_files:
#                 request_parts.append(types.Part.from_bytes(data=file.getvalue(), mime_type=file.type))
...
#                 temperature=0.0 # דטרמיניסטיות מוחלטת לפרודקשן
#             ),
#         )
        
#         if gemini_uploaded_file:
#             self.client.files.delete(name=gemini_uploaded_file.name)
...
#     """מחלקה שמצליבה את ממצאי ה-AI עם הצהרות ה-CRM, מוחקת ראיות מפלילות ומונעת מקלוד להתחכם"""
...
#                 effective_facts['commercial_or_industrial_equipment_detected']['evidence'] = "APPROVED_BY_CRM_DO_NOT_FLAG" # מילת קוד הרמטית שקלוד לא יכול לעקוף
...
# # ==========================================
# # 🧠 שלב 4: ה-Agent של קלוד (Logic by Claude 4.6 Sonnet) - Uncertainty Aware
# # ==========================================
...
#     def evaluate_risk(self, vision_facts: dict, crm_data: dict) -> dict:
#         prompt = f"""
...
#         3. You MUST NOT reference the visual_evidence_log or CRM text to overrule this. The premium has already been paid for this hazard. Flagging it again is a critical failure.
...
#         4. age_mismatch: Compare `estimated_age_years` (if not null) with CRM `property_details.year_built` (Calculate age from current year. Trigger if gap is > 15 yrs).
#         5. overcrowded: Is `estimated_occupants_based_on_beds_and_items` (if not null) > 10?
...
#         9. business_activity: Based on `commercial_or_industrial_equipment_detected`. Check if CRM `underwriting_questions.property_used_for_business` is true.
#         10. luxury: Based on `high_end_materials_or_smart_home_detected`. Check if CRM `property_characteristics.declared_finish_level` indicates luxury.
#         11. pool_or_jacuzzi: Based on `pool_or_jacuzzi_detected`.
#         12. area_exceeds_20_percent: Is `estimated_area_sqm` (if not null) > (CRM `property_characteristics.area_sqm` * 1.20)?
...
#         CRITICAL: Ensure your JSON is perfectly formatted. Do not use unescaped quotes inside strings.
#         {{
#             "flags": {{
...
#         # --- תיקון: חותך בדיוק מהסוגר הראשון ועד לסוגר האחרון ---
#         start_idx = response_text.find('{')
#         end_idx = response_text.rfind('}') + 1
        
#         if start_idx != -1 and end_idx != -1:
#             clean_json_string = response_text[start_idx:end_idx]
#         else:
...
# # ==========================================
# # 🎨 שלב 5: בונה הממשק ל-CRM (UI Builder)
# # ==========================================
# class UIBuilder:
#     @staticmethod
...
#             value = score,
#             domain = {'x': [0, 1], 'y': [0, 1]},
#             title = {'text': "ציון בטיחות וחיתום מבוסס אמון (Confidence)", 'font': {'size': 18}},
#             gauge = {
...
# # ==========================================
# # 🔄 שלב 6: המערכת הראשית (Main)
# # ==========================================
# def main_flow():
#     st.set_page_config(page_title="ביטוח ישיר - מערכת חיתום 2026", page_icon="🏢", layout="wide")
#     st.title("🏢 מנוע חיתום Enterprise: זיהוי פערים בזמן אמת (Uncertainty-Aware)")
    
#     # === ה-JSON המעודכן שלך משולב כאן כברירת מחדל ===
#     sample_crm_data = {
...
#     with st.sidebar:
#         st.header("📥 נתוני פוליסה נכנסים (CRM)")
#         crm_json_input = st.text_area("JSON מקורי:", value=json.dumps(sample_crm_data, indent=2, ensure_ascii=False), height=300)
#         try:
#             crm_data = json.loads(crm_json_input)
#         except json.JSONDecodeError:
#             st.error("שגיאת JSON.")
#             crm_data = {}
            
#         st.divider()
#         st.subheader("🔑 הגדרות API")
        
#         try:
#             gemini_key = st.secrets["GEMINI_API_KEY"]
#             claude_key = st.secrets["CLAUDE_API_KEY"]
#             st.success("✅ מפתחות API נטענו בהצלחה מה-Secrets")
#         except KeyError:
#             st.error("❌ מפתחות API חסרים בקובץ secrets.toml")
#             gemini_key = ""
#             claude_key = ""

#     st.markdown("### 📸 השלמת פוליסת הביטוח - העלאת מדיה")
#     st.info("על מנת להשלים את הפקת הפוליסה, המערכת דורשת תיעוד של החללים הבאים. ניתן להעלות תמונות נפרדות או סיור וידאו רציף:")
    
#     c1, c2, c3 = st.columns(3)
#     with c1:
#         st.write("✔️ כניסה לבניין")
#         st.write("✔️ צילום הדירה מבחוץ")
#         st.write("✔️ דלת כניסה")
#     with c2:
#         st.write("✔️ סלון")
#         st.write("✔️ מטבח")
#         st.write("✔️ חדרי שינה")
#     with c3:
#         st.write("✔️ גינה / מרפסת")
#         st.write("✔️ פרגולה (אם יש)")
#         st.write("✔️ מחסן (אם יש)")

#     st.divider()
    
#     upload_mode = st.radio("בחר אופן העלאה:", ["תמונות מרובות", "סרטון וידאו (סיור מקיף)"], horizontal=True)
    
#     uploaded_files, uploaded_video = None, None
#     if upload_mode == "תמונות מרובות":
#         uploaded_files = st.file_uploader("העלה את התמונות כאן (מינימום 3)", type=["jpg", "jpeg", "png", "webp"], accept_multiple_files=True)
#     else:
#         uploaded_video = st.file_uploader("העלה סרטון וידאו", type=["mp4", "mov"])

#     is_ready = (upload_mode == "תמונות מרובות" and uploaded_files) or (upload_mode == "סרטון וידאו" and uploaded_video)

#     if is_ready and st.button("נתח נתונים והפק דוח חיתום 🚀", type="primary"):
#         if not gemini_key or not claude_key:
#             st.error("אנא וודא שהמפתחות מוגדרים בקובץ secrets.toml.")
#             return
...
#         status_text.markdown("⏳ **שלב 1/3:** Gemini 3.1 Pro מבצע Multi-pass Analysis ומחשב Confidence...")
        
#         try:
#             gemini_agent = GeminiVisionAgent(api_key=gemini_key)
# # אנחנו מניחים שה-task_id מגיע משם הקובץ או בחירה, לצורך הדמו נשתמש במושג כללי או נגדיר משתנה
#             target_room = "סלון" # כאן את יכולה להחליף את זה בבחירה דינמית מהממשק
            
#             vision_facts = gemini_agent.process_and_analyze(upload_mode, uploaded_files, uploaded_video, expected_room=target_room)
            
#             # --- חסימה חדשה: בדיקת התאמה לחדר ---
#             if not vision_facts.get("is_expected_room", True):
#                 progress_bar.empty()
#                 status_text.empty()
#                 reason = vision_facts.get("room_mismatch_reason", "החדר לא תואם למבוקש.")
#                 st.error(f"❌ **טעות בזיהוי החדר:**\n\nהמערכת ציפתה לראות **{target_room}**, אך זיהתה משהו אחר.\n\n**פירוט:** {reason}", icon="🧐")
#                 return            
#             # ==========================================
#             # 🛑 חסימה קריטית: אם זה לא נכס, עוצרים הכל!
#             # ==========================================
#             if not vision_facts.get("is_valid_property", True):
#                 progress_bar.empty()
#                 status_text.empty()
#                 reason = vision_facts.get("invalid_property_reason", "התמונה לא תואמת נהלי חיתום.")
#                 st.error(f"🚨 **שגיאת אימות חמורה:**\n\nהמערכת זיהתה שהמדיה שהועלתה אינה מכילה נכס (דירה/בית) ולכן החיתום מופסק אוטומטית.\n\n**סיבת הדחייה שזוהתה:** {reason}", icon="🛑")
#                 return # יציאה מיידית מהפונקציה - לא ממשיכים לקלוד!

#         except Exception as e:
#             st.error(f"שגיאה בתהליך ה-Vision: {e}")
#             return
            
#         time_gemini = time.time() - start_time_gemini
#         progress_bar.progress(33)
                
#         status_text.markdown("⏳ **שלב 2/3:** מגשר ההצהרות מוודא את ממצאי ה-AI מול נתוני ה-CRM...")
#         try:
#             reconciler = DeclarationReconciler()
#             effective_facts, approved_declarations = reconciler.reconcile(vision_facts, crm_data)
#         except Exception as e:
#             st.error(f"שגיאה בתהליך ההשוואה מול ה-CRM: {e}")
#             return

#         progress_bar.progress(66)

#         start_time_claude = time.time()
#         status_text.markdown("⏳ **שלב 3/3:** Claude 4.6 Sonnet מבצע חיתום מוכוון-אי-ודאות...")
        
#         try:
#             claude_agent = ClaudeUnderwriterAgent(api_key=claude_key)
#             underwriting_result = claude_agent.evaluate_risk(effective_facts, crm_data)
#         except Exception as e:
#             st.error(f"שגיאה בהסקת המסקנות (Claude): {e}")
#             return

#         time_claude = time.time() - start_time_claude
#         progress_bar.progress(100)
#         status_text.markdown(f"✨ **התהליך הושלם!** (זמן עיבוד: {time_gemini + time_claude:.2f} שניות)")
        
#         st.subheader("⚖️ דוח חיתום סופי למערכת (CRM View)")

#         if approved_declarations:
#             st.success("✅ **התאמות CRM שאושרו:** הלקוח הצהיר בכנות על הסעיפים הבאים (הקנסות בוטלו לפני החיתום):")
#             for app in approved_declarations:
#                 st.write(app)

#         col_score, col_dec = st.columns([1, 1.5])
        
#         score = underwriting_result.get("score", 0)
#         decision = underwriting_result.get("decision", "")
        
#         with col_score:
#             UIBuilder.display_gauge(score)
#             if decision == "Approve": st.success("✅ מאושר אוטומטית (Green Light)")
#             elif decision == "Manual Review": st.warning("⚠️ דרושה התערבות חתם")
#             else: st.error("❌ נדחה / סיכון גבוה")
            
#         with col_dec:
#             st.write("### 📝 נימוק המערכת:")
#             st.info(underwriting_result.get("reasoning_for_crm", "לא התקבל נימוק."))
            
#             st.write("### 🚩 דגלים אדומים וחריגות שזוהו (רק לאחר קיזוז CRM):")
#             flags = underwriting_result.get("flags", {})
            
#             has_alerts = False
#             for flag_name, flag_data in flags.items():
#                 if flag_data.get("triggered"):
#                     has_alerts = True
#                     conf_status = "🔴 ודאות גבוהה" if flag_data.get("is_high_confidence") else "🟠 חשד בלבד (ודאות נמוכה)"
#                     st.write(f"- **{flag_name}** [{conf_status}]: {flag_data.get('evidence', 'אין פירוט')}")
            
#             if not has_alerts:
#                 st.write("- ✅ לא נמצאו חריגות מהותיות מהפוליסה. הנתונים תואמים.")

#             with st.expander("🔍 צפייה בראיות החזותיות הגולמיות (Audit Trail) לפני ואחרי גישור"):
#                 st.write("**ראיות אחרי גישור (מה שקלוד קיבל):**")
#                 st.json(effective_facts)
...
#     def validate_input_quality(self, files) -> bool:
#         """סימולציה של בקרת איכות קלט (Blur/Brightness) - נדרש לפרודקשן"""
#         if not files or len(files) < 3: 
#             return False
#         return True

#     def process_and_analyze(self, upload_mode, uploaded_files, uploaded_video):
#         if upload_mode == "תמונות מרובות" and not self.validate_input_quality(uploaded_files):
#             raise Exception("Input Quality Error: נדרשות לפחות 3 תמונות ברורות לניתוח מהימן.")

#         request_parts = [types.Part.from_text(text=self.build_prompt())]
#         gemini_uploaded_file = None
...
#         try:
#             gemini_agent = GeminiVisionAgent(api_key=gemini_key)
#             vision_facts = gemini_agent.process_and_analyze(upload_mode, uploaded_files, uploaded_video)
...
#     def __init__(self, api_key: str):
#         self.client = anthropic.Anthropic(api_key=api_key)
#         self.model_name = "claude-3-5-sonnet-20241022"  # מעודכן למודל הזמין ב-API
